...
static int hwspin_lock_test_anonymous_locks(struct platform_device *pdev)
{
	int i;
	int ret = 0, ret1 = 0;
	int max_locks;
...
static int omap_hwspinlock_test_probe(struct platform_device *pdev)
{
	...
	if (ret)
		pr_err("hwspin_lock_test_all_locks failed, ret = %d\n", ret);
	pr_err("\n***** End - Test All Specific Locks ****\n");

	pr_err("\n***** Begin - Test All Anonymous Locks ****\n");
	ret = hwspin_lock_test_anonymous_locks(pdev);
	if (ret)
		pr_err("hwspin_lock_test_anonymous_locks failed, ret = %d\n", ret);
	pr_err("\n***** End - Test All Anonymous Locks ****\n");
